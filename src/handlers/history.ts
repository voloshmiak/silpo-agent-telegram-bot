...
historyHandler.hears("📜 Історія", async (ctx) => {
    ...
    try {
        ...
        // 🛡 Валідація: якщо бекенд повернув пустоту, null або пустий масив
        if (!plans || !Array.isArray(plans) || plans.length === 0) {
            return ctx.api.editMessageText(
                ctx.chat!.id,
                loading.message_id,
                "📭 *У вас ще немає збережених раціонів.*\nЗгенеруйте свій перший план!",
                { parse_mode: "Markdown" }
            );
        }
        ...
        // 🛡 Якщо бекенд кидає помилку на відсутність планів (напр. 404)
        await ctx.api.editMessageText(
            ctx.chat!.id,
            loading.message_id,
            "📭 *У вас ще немає збережених раціонів.*\nАбо сталася помилка з'єднання.",
            { parse_mode: "Markdown" }
        );
    }
});

// Обробник натискання на кнопку детального перегляду
